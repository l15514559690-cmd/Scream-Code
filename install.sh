...
scream() {{
  (
    ...
    . {act.as_posix()!r}
    # 裸命令 scream（无参数）且已安装 Rust 客户端时 → 全屏 TUI；否则走 Python（含 scream config / scream repl 等）
...
echo ""
echo "提示: 配置模型与密钥请运行 scream config（Python）。裸 scream 若已 cargo install scream-cli，会进 Rust 全屏 TUI。本地 llm_config.json / .env 勿提交；可先 cp llm_config.json.example llm_config.json。"
