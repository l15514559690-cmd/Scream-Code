...
def extract_claims(material: str) -> List[str]:
    ...
    for sent in re.split(r'[。！？\n]', material):
        ...
        # 检查是否包含地理关键词或数据
        has_keyword = any(kw in sent for kw in GEO_KEYWORDS)
        has_data = re.search(r'[\d,.%％]+(?:米|公里|km|KM|℃|°|万|亿|千|ha|km²)', sent)
        
        if has_keyword or has_data:
            # 清理序号前缀
...
def classify_source(url: str) -> Dict[str, Any]:
    ...
    # 官方机构
    official_domains = ['gov.cn', 'gov.', 'nasa.gov', 'noaa.gov', 'stats.gov.cn', 
                        'cma.cn', 'un.org', 'who.int', 'unesco.org']
    if any(d in url_lower for d in official_domains):
        return {"type": "官方", "weight": 1.0}
    
    # 学术期刊
    academic_domains = ['cnki.net', 'wanfangdata', 'springer', 'nature.com', 
                        'sciencedirect', 'arxiv', 'doi.org', 'jstor.org']
    if any(d in url_lower for d in academic_domains):
        return {"type": "学术", "weight": 0.95}
    
    # 权威媒体
    media_domains = ['xinhuanet', 'people.com.cn', 'cctv', 'bbc.com', 
                     'reuters', 'thepaper', 'caixin', 'guardian', 'nytimes']
    if any(d in url_lower for d in media_domains):
        return {"type": "权威媒体", "weight": 0.85}
    
    # 专业媒体
    professional_domains = ['sciencenet', 'dili', 'nationalgeographic', 
                            'smithsonian', 'earth', 'space.com', 'dili360',
                            'geology.com', 'nasa.gov', 'esa.int']
    if any(d in url_lower for d in professional_domains):
        return {"type": "专业媒体", "weight": 0.75}
    
    # 百科
    wiki_domains = ['wikipedia', 'wikimedia', 'baike.baidu', 'wikidata', 
                   'britannica', 'baike.sogou', 'baike.qq', 'baike.360', 'osgeo']
    if any(d in url_lower for d in wiki_domains):
        return {"type": "百科", "weight": 0.65}
    
    # 社交媒体
    social_domains = ['weibo', 'weixin', 'douyin', 'zhihu', 'twitter', 
                     'facebook', 'toutiao', 'bilibili', 'reddit']
...
def search_ddg(query: str, max_results: int = 10) -> List[Dict]:
    ...
    try:
        ...
        print("   ⚠️ 请先安装 ddgs: pip install ddgs")
        return []
...
def calculate_score(sources: List[Dict]) -> Dict[str, Any]:
    ...
    # 评级描述
    if score >= 85:
        level, desc = "高度客观", "多个权威来源支持，客观性很高"
    elif score >= 70:
        level, desc = "较为客观", "有可靠来源支撑，客观性较好"
    elif score >= 50:
        level, desc = "部分客观", "来源参差不齐，需谨慎参考"
    elif score >= 30:
        level, desc = "可信存疑", "缺乏权威来源支撑"
    else:
        level, desc = "难以判断", "未找到有效来源"
...
def check_geography_material(material: str, output_path: str = None) -> str:
    ...
    Example:
        >>> report = check_geography_material("珠穆朗玛峰海拔8848米")
        >>> print(report)
    """
    print("=" * 60)
    print("🔍 地理素材核查助手-by李李老师 v1.7.0")
    print("=" * 60)
    ...
    if output_path is None:
        ...
        output_path = os.path.join(desktop, "地理素材核查报告.md")
...
if __name__ == "__main__":
    ...
    print("=" * 50)
    print("📖 使用方法:")
    print("   from geo_fact_checker_by_lili import check_geography_material")
    print("   check_geography_material('珠穆朗玛峰海拔8848米')")
    print("=" * 50)
