...
def execute_command(name: str, prompt: str = '') -> CommandExecution:
    ...
    if module is None:
        return CommandExecution(name=name, source_hint='', prompt=prompt, handled=False, message=f'未知镜像命令: {name}')
    action = f"镜像命令「{module.name}」（来源 {module.source_hint}）将处理提示 {prompt!r}。"
...
def render_command_index(limit: int = 20, query: str | None = None) -> str:
    ...
    lines = [f'命令条目: {len(PORTED_COMMANDS)}', '']
    if query:
        lines.append(f'筛选条件: {query}')
        lines.append('')
