...
def _coerce_boolish(v: Any) -> bool:
...
class ModelProfile:
    ...
    #: 与 TUI「深度思考中…」等文案联动；见 ``llm_settings.is_model_deep_thinking_enabled``。
        ...
        try:
            ...
            out.append(
                ModelProfile(
                    ...
                    api_protocol=proto,
                    deep_thinking=_coerce_boolish(item.get('deep_thinking', False)),
                )
            )
        except KeyError:
            continue
...
def add_model(profile: ModelProfile) -> None:
    ...
    profile = ModelProfile(
        ...
        deep_thinking=profile.deep_thinking,
    )
    raw.setdefault('models', [])
            ...
            new_list.append(
                asdict(
                    ModelProfile(
                        ...
                        deep_thinking=new_profile.deep_thinking,
                    )
                )
            )
            found = True
