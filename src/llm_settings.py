...
def is_model_deep_thinking_enabled() -> bool:
    """
    是否启用「模型深度思考」模式（用于 TUI「深度思考中…」等文案）。

    优先级：

    1. 环境变量 ``SCREAM_MODEL_DEEP_THINKING`` 或 ``MODEL_DEEP_THINKING``：显式 ``1``/``true``/``yes``/``on``
       为开启，``0``/``false``/``no``/``off`` 为关闭（覆盖配置文件）。
    2. ``~/.scream/llm_config.json`` 当前激活模型的 ``deep_thinking``。
    3. 配置根字段 ``deep_thinking``。
    """
    load_project_dotenv()
    for key in ('SCREAM_MODEL_DEEP_THINKING', 'MODEL_DEEP_THINKING'):
        raw = read_project_dotenv_value(key)
        if not raw:
            continue
        s = raw.strip().lower()
    ...
    try:
        ...
        model_manager.ensure_default_config_file()
        cfg = model_manager.read_persisted_config_raw()
        if cfg is None:
            return False
        prof = model_manager.get_active_profile(cfg)
        if prof is not None and getattr(prof, 'deep_thinking', False):
            return True
        root = cfg.get('deep_thinking')
        if isinstance(root, bool) and root:
            return True
        if isinstance(root, str) and root.strip().lower() in ('1', 'true', 'yes', 'on'):
            return True
    except Exception:
        pass
    return False
