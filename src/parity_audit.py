...
class ParityAuditResult:
    ...
    def to_markdown(self) -> str:
        ...
        if not self.archive_present:
            lines.append('本地归档不可用；无法与原始 TypeScript 快照进行一致性对照。')
            return '\n'.join(lines)

        lines.extend([
            '',
            f'根文件覆盖: **{self.root_file_coverage[0]}/{self.root_file_coverage[1]}**',
            f'目录覆盖: **{self.directory_coverage[0]}/{self.directory_coverage[1]}**',
            f'Python 文件总数 / 归档中类 TS 文件数: **{self.total_file_ratio[0]}/{self.total_file_ratio[1]}**',
            f'命令条目覆盖: **{self.command_entry_ratio[0]}/{self.command_entry_ratio[1]}**',
            f'工具条目覆盖: **{self.tool_entry_ratio[0]}/{self.tool_entry_ratio[1]}**',
            '',
            '缺失的根级目标:',
        ])
