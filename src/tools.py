...
def execute_tool(name: str, payload: str = '') -> ToolExecution:
    ...
    if module is None:
        return ToolExecution(name=name, source_hint='', payload=payload, handled=False, message=f'未知镜像工具: {name}')
    action = f"镜像工具「{module.name}」（来源 {module.source_hint}）将处理负载 {payload!r}。"
