...
# 思考链占位（<thinking>）期间 Live 仅用 Spinner，按时间刷新帧以转动动画
STREAM_LIVE_THINKING_ANIM_INTERVAL_SEC = 1.0 / 12.0
...
def strip_thinking_blocks_for_display(text: str) -> str:
    """
    从用于 Live Markdown 的缓冲中移除已闭合的 ``<thinking>...</thinking>`` 块，
    避免流式结束后仍把隐藏推理当正文重排（减轻卡顿与乱码标签）。
    """
    import re
...
def streaming_markdown_for_live(buffer: str, *, console: Any | None = None) -> Any:
    ...
    **仅**供 ``rich.Live`` 使用：轻量左边框 ``Panel`` + ``ScreamMarkdown``，强化流式区与历史区的视觉隔离。
    ...
    buf = strip_thinking_blocks_for_display(buffer or '')
    buf = prepare_streaming_live_buffer(buf, console=console)
