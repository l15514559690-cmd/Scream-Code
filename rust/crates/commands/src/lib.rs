...
const SLASH_COMMAND_SPECS: &[SlashCommandSpec] = &[
    ...
    SlashCommandSpec {
        name: "memo",
        aliases: &[],
        summary: "Long-term memory (Scream Code)",
...
pub enum SlashCommand {
    ...
    /// 群狼 / 多代理模式（Scream Code 行内 REPL；TUI 仍走 Python）。
    Team,
    /// 长效记忆（Scream Code）。
    Memo {
        text: Option<String>,
    },
    /// 停止当前生成（TUI 中由 Python 处理；行内 REPL 仅提示）。
    Stop,
    Unknown(String),
}
...
fn slash_command_category(name: &str) -> &'static str {
    match name {
        ...
        "model" | "permissions" | "config" | "memory" | "memo" | "theme" | "vim" | "voice"
