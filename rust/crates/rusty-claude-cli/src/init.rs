...
const GITIGNORE_COMMENT: &str = "# Scream Code local artifacts";
...
pub(crate) fn render_init_claude_md(cwd: &Path) -> String {
    ...
    let mut lines = vec![
        "# CLAUDE.md".to_string(),
        String::new(),
        "This file provides guidance to Scream Code when working with code in this repository."
            .to_string(),
        String::new(),
    ];
...
mod tests {
    ...
    fn temp_dir() -> std::path::PathBuf {
        let nanos = SystemTime::now()
            ...
            .as_nanos();
        std::env::temp_dir().join(format!("scream-cli-init-{nanos}"))
    }
